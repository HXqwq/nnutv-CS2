...
export function loadMatches() {
  ...
  // 排序：date 倒序，同日内 time 倒序，无 time 的排到该日最后
...
export function h2hMatrix(matchId) {
  ...
  // 只有"真正带对位数据"的选手才参与建表：h2h 为空对象 {} 时视作没有对位数据。
  // 否则整场都缺对位数据时会渲染出一张全 0 的空表（如本站无对位接口的场次）。
  const hasH2h = (s) => !!s.h2h && Object.keys(s.h2h).length > 0;

  for (const map of match.maps) {
    const teamAStats = map.stats.filter((s) => s.side === "A" && hasH2h(s));
    const teamBStats = map.stats.filter((s) => s.side === "B" && hasH2h(s));
...
// 最近一周的 PoW 选手（按该周内的所有地图 stat 行 rating 取均值），返回 {player, agg, week, fallback, range}
// fallback=true 表示最近 7 天没有比赛，回退到最近一场比赛所在周
// range 形如 {start: '2026-04-09', end: '2026-04-15'}
// 算法：以"今天"为基准，逐级扩大窗口（7天 → 30天 → 整赛季），取首个含 rating 数据的窗口；
// 若全无数据（骨架比赛 stats 空）则 null
export function getPlayerOfTheWeek() {
  ...
  // 找含 rating 数据的比赛日（升序）
  const datesWithData = [...new Set(matches
    .filter((m) => (m.maps || []).some((mp) => (mp.stats || []).some((s) => typeof s.rating === "number")))
    .map((m) => m.date))]
    .sort();
  if (datesWithData.length === 0) return null;
  const latestDataDate = datesWithData[datesWithData.length - 1];
  const latestDataMs = Date.parse(latestDataDate);

  // 窗口候选：以"今天"为基准 7 天；以"今天"为基准 30 天；以"今天"为基准 90 天；最后用"含数据最近一天"为 end 的 7 天
  const now = new Date();
  const nowMs = now.getTime();
  const windows = [
    { start: nowMs - 7 * 86400000,    end: nowMs,              fallback: false, label: "今日 ±7 天" },
    { start: nowMs - 30 * 86400000,   end: nowMs,              fallback: true,  label: "今日 ±30 天" },
    { start: nowMs - 90 * 86400000,   end: nowMs,              fallback: true,  label: "今日 ±90 天" },
    { start: latestDataMs - 7 * 86400000, end: latestDataMs,  fallback: true,  label: "最近数据 ±7 天" },
  ];

  let range, fallback;
  let candidates = [];
  for (const w of windows) {
    candidates = matches.filter((m) => {
      const ms = Date.parse(m.date);
      return ms >= w.start && ms <= w.end;
    }).filter((m) => (m.maps || []).some((mp) => (mp.stats || []).length > 0));
    if (candidates.length > 0) {
      const inRange = candidates.map((m) => m.date).sort();
      range = { start: inRange[0], end: inRange[inRange.length - 1] };
      fallback = w.fallback;
      break;
    }
  }
  ...
  // 收集该窗口内的 stat 行，按 playerId 聚合
  const inWindowMatches = matches.filter((m) => m.date >= range.start && m.date <= range.end);
  const agg = new Map();
  for (const p of players) {
    agg.set(p.id, { player: p, maps: 0, ratingSum: 0, ratingN: 0, kills: 0, deaths: 0, adrSum: 0, adrN: 0 });
  }
  for (const m of inWindowMatches) {
    for (const map of m.maps || []) {
      for (const s of (map.stats || [])) {
        const a = agg.get(s.playerId);
        if (!a) continue;
  ...
  const candidates2 = [...agg.values()]
    .filter((a) => a.maps > 0 && a.ratingN > 0)
    .map((a) => ({
      ...a,
      avgRating: a.ratingSum / a.ratingN,
      avgAdr: a.adrN ? a.adrSum / a.adrN : null,
    }))
    .sort((x, y) => y.avgRating - x.avgRating);

  if (candidates2.length === 0) return null;
  return { ...candidates2[0], week: range, fallback };
}

// 最近转会记录（按 date 倒序），可限制条数
export function getRecentTransfers(limit = 6) {
  const transfers = loadTransfers();
  return [...transfers]
    .sort((a, b) => (b.date || "").localeCompare(a.date || ""))
    .slice(0, limit);
...
export function playerH2HHistory(playerId) {
  ...
  for (const m of matches) {
    for (const map of m.maps) {
      const s = map.stats.find((x) => x.playerId === playerId);
      // 空对象 {} 视作"没有对位数据"，否则会在选手页渲染出一个空的「对位数据」面板
      if (!s || !s.h2h || Object.keys(s.h2h).length === 0) continue;
      rows.push({
        matchId: m.id,
        date: m.date,
        map: map.map,
